...
- **Interpreting Recency & Freshness**: Every `FileResult` includes a `recency_signal` (float64, 0.0 to 1.0). A high signal (>0.4) indicates an **Active Frontier** undergoing significant development, which agents should prioritize for review. A low signal (<0.1) indicates a **Historical Hotspot** driven by past volume; these should be viewed as "stable legacy" rather than active bottlenecks. Because of the **Recency Paradox**, the lifetime Rank 1 in a young repo may often be a historical hotspot, so agents MUST verify this signal before recommending refactors.
