...
func computeScore(m *schema.FileMetrics, mode string) float64 {
	...
	if strings.Contains(m.Path, "test") || strings.Contains(m.Path, "generate") || strings.Contains(m.Path, "example") {
		if modeLower == "risk" {
			score *= 0.75
		} else {
			score *= 0.50
	...
	if strings.Contains(m.Path, "yml") || strings.Contains(m.Path, "yaml") || strings.Contains(m.Path, "json") {
