...
// RecordFileResultsBatch inserts multiple file metrics and scores into PostgreSQL using a single transaction.
func (d *PostgresDialect) RecordFileResultsBatch(db *sql.DB, tableName string, analysisID int64, results []schema.BatchFileResult) error {
	tx, err := db.Begin()
	if err != nil {
		return err
	}
	...
	for _, res := range results {
		...
		_, err := stmt.Exec(
			...
			res.Scores.HotScore, res.Scores.RiskScore, res.Scores.ComplexityScore, res.Scores.ROIScore, res.Scores.ScoreLabel, reasoningJSON,
