...
func (c *LocalGitClient) GetActivityLog(ctx context.Context, repoPath string, startTime, endTime time.Time) ([]byte, error) {
	...
	if !startTime.IsZero() {
		args = append(args, fmt.Sprintf("--since=%s", startTime.Format(schema.DateTimeFormat)))
	}
	if !endTime.IsZero() {
...
func (c *LocalGitClient) GetFileActivityLog(ctx context.Context, repoPath string, path string, startTime, endTime time.Time, follow bool) ([]byte, error) {
	...
	if follow {
		...
		if !startTime.IsZero() {
			args = append(args, "--since="+startTime.Format(schema.DateTimeFormat))
		}
		if !endTime.IsZero() {
			args = append(args, "--until="+endTime.Format(schema.DateTimeFormat))
		}
	}
